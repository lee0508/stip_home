...
// 인증 응답 Signature = hex(sha256(AuthToken + MID + Amt + MerchantKey) // 실제 거래시
...
if($authResultCode === "0000" /* && $authSignature == $authComparisonSignature*/){
	...
	*/	
	...
	try{
		...
		// 결제 상태 업데이트
		$paymentData = [
			'payment_id' => $_POST['Moid'], // ORD_ 제거 필요
			'status' => 'completed',
			'TID' => $txTid,
		...
		// 결제 상태 업데이트 요청
		$updateResponse = reqPost($paymentData, 'update_payment_status.php');

		// 클라이언트에 결과 전송
		echo "<script>
            if (window.opener) {
                window.opener.paymentHandler.handlePaymentSuccess(" . json_encode($response) . ");
                window.close();
            }
        </script>";
	...
	echo "<script>
        if (window.opener) {
            window.opener.paymentHandler.handlePaymentError(" . json_encode([
